...
final class Orion_AI_Evaluation_Service{
 public function run(array $settings,string $suite_file):array{
  ...
  foreach($cases as$case){
   ...
   $actual_intent=(string)($last['intent']??'');$expected_intents=is_array($case['expected_intents']??null)?$case['expected_intents']:array();if(isset($case['expected_intent']))$expected_intents[]=(string)$case['expected_intent'];$expected_intents=array_values(array_unique(array_filter(array_map('strval',$expected_intents))));if($expected_intents&&!in_array($actual_intent,$expected_intents,true))$failures[]='intent';
   ...
   $actual_roles=array();foreach($actual_plan as$item){$role=(string)($item['role']??'');$hint=(string)($item['role_hint']??'');if($role!=='')$actual_roles[]=$role;if($hint!=='')$actual_roles[]=$hint;$known=Orion_Role_Registry::role_hint($role);if($known!=='')$actual_roles[]=$known;}$actual_roles=array_values(array_unique($actual_roles));
   foreach(($case['expected_roles']??array())as$role)if(!in_array($role,$actual_roles,true))$failures[]='missing_role.'.$role;
   foreach(($case['expected_optional_roles']??array())as$role){$optional=false;foreach($actual_plan as$item){$item_role=(string)($item['role']??'');$item_hint=(string)($item['role_hint']??'');if(($item_role===$role||$item_hint===$role||Orion_Role_Registry::role_hint($item_role)===$role)&&empty($item['required'])){$optional=true;break;}}if(!$optional)$failures[]='optional_role.'.$role;}
   ...
   $details=$ok?'':wp_json_encode(array('failures'=>$failures,'expected_intents'=>$expected_intents,'actual_intent'=>$actual_intent,'expected_state'=>$case['expected_state']??array(),'actual_state'=>$state,'expected_roles'=>$case['expected_roles']??array(),'actual_roles'=>$actual_roles,'expected_need_keywords'=>$case['expected_need_keywords']??array(),'actual_needs'=>$actual_plan),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
   $rows[]=array('case'=>sanitize_text_field((string)($case['name']??'Unnamed')),'result'=>$ok?'PASS':'FAIL','intent'=>$actual_intent,'diagnostic'=>$diagnostic,'duration_ms'=>$duration,'details'=>$details);
  }
