...
final class Orion_Semantic_Product_Planner {
    ...
    public function recommend(array $classification, array $state, array $settings): array {
        ...
        foreach ($plan as $item) {
            ...
            foreach ($found as $product) {
                $id = (int) ($product['id'] ?? 0);
                if (!$id) { continue; }
                $candidate_ids[$need_key][$id] = true;
                $ids[] = $id;
                if (!isset($catalogue[$id])) {
                    $catalogue[$id] = array(
                        ...
                        'retrieval'=>$product['retrieval'] ?? array(),
                    );
                }
        ...
        if (!$candidate_ids) {
            ...
            return array(
                ...
                'conditional_roles'=>array_values(array_unique($conditional_roles)),
                'plan'=>$groups,
                'diagnostic'=>array('status'=>'no_relevant_candidates','catalogue_products'=>0,'need_candidates'=>0,'open_need_contract'=>true),
            );
        }
        ...
        $tool = array('type'=>'function','function'=>array(
            ...
            'parameters'=>array('type'=>'object','additionalProperties'=>false,'properties'=>array(
                'selections'=>array('type'=>'array','maxItems'=>20,'items'=>array('type'=>'object','additionalProperties'=>false,'properties'=>array(
                    'need_key'=>array('type'=>'string','enum'=>$selection_need_keys),
                    'product_id'=>array('type'=>'integer'),
                    'reason'=>array('type'=>'string'),
                    'evidence_fields'=>array('type'=>'array','minItems'=>1,'maxItems'=>7,'items'=>array('type'=>'string','enum'=>Orion_Selection_Evidence::fields())),
                    'confidence'=>array('type'=>'string','enum'=>array('high','medium','low')),
                    'uncertainty'=>array('type'=>'string'),
                ),'required'=>array('need_key','product_id','reason','evidence_fields','confidence','uncertainty')),
                'missing_needs'=>array('type'=>'array','items'=>array('type'=>'string','enum'=>$selection_need_keys)),
            ),'required'=>array('selections','missing_needs')),
        ));
        ...
        if (is_array($data)) {
            foreach (($data['selections'] ?? array()) as $choice) {
                ...
                if (isset($selected_index[$id])) {
                    ...
                    $index = $selected_index[$id];
                    $this->add_role($selected[$index], $need_key, $role_hint);
                ...
                $product['recommendation_reason'] = $evidence['reason'];
                $product['selection_evidence'] = array($evidence_entry);
                $selected_index[$id] = count($selected);
                $selected[] = $product;
            }
        }
        ...
        $selected_before_limit = count($selected);
        ...
        foreach ($displayed as $product) {
            foreach ((array) ($product['logical_roles'] ?? array()) as $role) { if ($role !== '') { $displayed_roles[] = $role; } }
        }
        $missing = array();
        foreach ($groups as $group) {
            if (empty($group['conditional']) && !in_array($group['need_key'], $displayed_roles, true)) { $missing[] = $group['need_key']; }
        }
        ...
        if (empty($result['ok'])) {
            ...
            $diagnostic = array(
                'status'=>'ok','usage'=>$result['usage'] ?? array(),'attempts'=>$result['attempts'] ?? array(),'fallback_used'=>!empty($result['fallback_used']),
                'catalogue_products'=>count($catalogue),'need_candidates'=>array_sum(array_map('count', array_column($needs_payload,'candidate_ids'))),
                ...
                'roller_system_complete'=>in_array('roller',$displayed_roles,true),'selection_rejections'=>$rejections,'selector_missing_needs'=>$reported_missing,
                'deterministic_recovery'=>$recovery,'quantity_guidance'=>$this->quantity_diagnostics($displayed),'open_need_contract'=>true,'selection_evidence_required'=>true,
            );
        }
    ...
    private function normalise_plan(array $plan, array $state): array {
        ...
        usort($out, static function ($first, $second): int {
            $first_role = (string) ($first['role_hint'] ?: $first['need_key'] ?? '');
            $second_role = (string) ($second['role_hint'] ?: $second['need_key'] ?? '');
    ...
    private function search_candidates(array $item, string $need_key, string $role_hint, array $state, int $limit): array {
        ...
        foreach ((array) ($item['search_terms'] ?? array()) as $term) {
            $term = sanitize_text_field((string) $term);
            if ($term !== '') { $queries[] = $term; }
        }
        $validation_role = $role_hint ?: $need_key;
        if ('roller' === $validation_role) {
            $surface = sanitize_text_field((string) ($state['surface'] ?? ''));
            $queries[] = 'complete paint roller set with frame and sleeve '.$surface;
            $queries[] = 'paint roller frame '.$surface;
            $queries[] = 'paint roller sleeve '.$surface;
        }
        $found = array();
        foreach (array_unique(array_filter($queries)) as $candidate_query) {
            foreach ($this->products->search(array('query'=>$candidate_query,'role'=>$validation_role,'requirements'=>$requirements,'in_stock'=>true,'limit'=>$limit)) as $product) {
                $id = (int) ($product['id'] ?? 0);
                if ($id && !isset($found[$id]) && $this->candidate_allowed($product, $validation_role, $state)) { $found[$id] = $product; }
            }
        }
        if ('roller' === $validation_role) {
            uasort($found, function ($first, $second): int {
                $first_meta = $this->facts->roller_component($first); $second_meta = $this->facts->roller_component($second);
                $weight = array('complete'=>300,'frame'=>200,'sleeve'=>190,'unknown'=>0);
    ...
    private function candidate_allowed(array $product, string $role, array $state): bool {
        $facts = $this->facts->extract($product); $functions = $facts['functions'];
        $identity = strtolower((string) ($product['name'] ?? '').' '.implode(' ', (array) ($product['categories'] ?? array())));
    ...
    private function recover_roller_system(array $selected, array $candidate_ids, array $state): array {
        ...
        if ($complete) {
            ...
            foreach ($selected as $index => $product) {
                if ((int) ($product['id'] ?? 0) === $id && $this->facts->is_bundle($product)) {
                    $this->add_role($selected[$index],'roller','roller');
                    return array($selected,array('roller_complete_set'=>$id));
                }
            }
            $product = $this->decorate_selection($complete[0]['product'],'roller','roller');
            $product['recommendation_reason'] = 'Selected by deterministic complete roller-system recovery.';
            $product['selection_evidence'] = array($this->deterministic_evidence('roller','Explicit frame-and-sleeve bundle facts support this recovery.'));
            $selected[] = $product;
            return array($selected,array('roller_complete_set'=>$id));
        }
        foreach ($frames as $frame) { foreach ($sleeves as $sleeve) {
            ...
            $frame_product = $this->decorate_selection($frame['product'],'roller','roller');
            $frame_product['recommendation_reason'] = 'Selected as the compatible roller frame.';
            $frame_product['selection_evidence'] = array($this->deterministic_evidence('roller','Explicit roller component type and width support this frame.'));
            $sleeve_product = $this->decorate_selection($sleeve['product'],'roller','roller');
            $sleeve_product['recommendation_reason'] = 'Selected as the width-compatible roller sleeve.';
            $sleeve_product['selection_evidence'] = array($this->deterministic_evidence('roller','Explicit roller component type and width support this sleeve.'));
    ...
    private function recover_core_roles(array $selected, array $candidate_ids, array $state, int $max_products): array {
        ...
        foreach ($roles as $role) {
            $has_role = false;
            foreach ($selected as $product) { if (in_array($role,(array)($product['logical_roles'] ?? array()),true)) { $has_role = true; break; } }
            ...
            foreach (array_keys((array) $candidate_ids[$role]) as $id) {
                ...
                $existing = null;
                foreach ($selected as $index => $item) { if ((int)($item['id'] ?? 0) === (int)$id) { $existing = $index; break; } }
                if (null !== $existing) {
                    if (!$this->facts->is_bundle($product)) { continue; }
                    $this->add_role($selected[$existing],$role,$role);
                    $selected[$existing]['selection_evidence'][] = $this->deterministic_evidence($role,'Explicit bundle contents support this known product need.');
                    $recovered[$role] = (int) $id; break;
                }
                if (count($selected) >= $max_products) { break; }
                $product = $this->decorate_selection($product,$role,$role);
                $product['recommendation_reason'] = 'Added from a verified catalogue candidate to complete the requested core kit.';
                $product['selection_evidence'] = array($this->deterministic_evidence($role,'Structured catalogue facts explicitly identify this product function.'));
    ...
    private function quantity_diagnostics(array $products): array {
        $out = array();
        foreach ($products as $product) { if (!empty($product['quantity_guidance'])) { $out[] = array('id'=>(int)($product['id'] ?? 0),'guidance'=>$product['quantity_guidance']); } }
        return $out;
    }

    private function decorate_selection(array $product, string $need_key, string $role_hint = ''): array {
        $facts = $this->facts->extract($product); $validation_role = $role_hint ?: $need_key; $display = $validation_role;
        if ('roller' === $validation_role) { $display = array('frame'=>'roller_frame','sleeve'=>'roller_sleeve','complete'=>'roller')[$facts['roller_component']] ?? 'roller'; }
        $product['kit_role'] = $display;
        $product['kit_roles'] = array($display);
        $product['logical_roles'] = array($need_key);
        $product['need_key'] = $need_key;
        $product['role_hint'] = $role_hint;
        $product['orion_facts'] = $facts;
        if ('roller' === $validation_role) { $product['roller_component'] = $facts['roller_component']; $product['roller_width_inches'] = $facts['roller_width_inches']; }
        if ('tray' === $validation_role) { $product['tray_width_inches'] = $facts['tray_width_inches']; }
        return $product;
    }

    private function add_role(array &$product, string $need_key, string $role_hint = ''): void {
        $validation_role = $role_hint ?: $need_key; $display = $validation_role;
        if ('roller' === $validation_role) {
            $kind = $this->facts->roller_component($product)['kind'];
            $display = array('frame'=>'roller_frame','sleeve'=>'roller_sleeve','complete'=>'roller')[$kind] ?? 'roller';
        }
        if (!in_array($display,(array)($product['kit_roles'] ?? array()),true)) { $product['kit_roles'][] = $display; }
        if (!in_array($need_key,(array)($product['logical_roles'] ?? array()),true)) { $product['logical_roles'][] = $need_key; }
        $product['kit_role'] = $product['kit_roles'][0] ?? $display;
    }

    private function deterministic_evidence(string $need_key, string $reason): array {
        return array('need_key'=>$need_key,'evidence_fields'=>array('name','facts'),'confidence'=>'high','reason'=>$reason,'uncertainty'=>'');
    }
    ...
    private function priority(array $product): int {
        $values = array_map(static fn($role)=>Orion_Role_Registry::priority((string)$role),(array)($product['kit_roles'] ?? array($product['kit_role'] ?? '')));
        return $values ? min($values) : 80;
    }

    private function empty_result(string $status): array {
        return array('products'=>array(),'missing_roles'=>array(),'missing_needs'=>array(),'conditional_roles'=>array(),'plan'=>array(),'diagnostic'=>array('status'=>$status,'open_need_contract'=>true));
    }
