...
final class Orion_AI_Settings {
    ...
    public static function defaults(): array { return array(
        'enabled'=>'1','api_key'=>'','model'=>'openrouter/free','primary_color'=>'#2783DE','position'=>'right','title'=>'Orion AI Assistant',
        ...
        'questions_per_session'=>5,'sessions_per_day'=>2,'session_minutes'=>30,'requests_per_minute'=>12,'max_products'=>6,'retention_days'=>30,
    ...
    public static function get(): array { return wp_parse_args( get_option( self::OPTION, array() ), self::defaults() ); }
    public static function sanitize( $input ): array {
        $input = is_array( $input ) ? $input : array(); $old = self::get(); $out = self::defaults();
        $out['enabled'] = empty( $input['enabled'] ) ? '0' : '1';
        $out['api_key'] = ! empty( $input['api_key'] ) ? sanitize_text_field( wp_unslash( $input['api_key'] ) ) : (string) ( $old['api_key'] ?? '' );
        foreach ( array( 'model','title','greeting','limit_message','system_prompt' ) as $key ) $out[$key] = sanitize_textarea_field( wp_unslash( $input[$key] ?? $out[$key] ) );
        $out['primary_color'] = sanitize_hex_color( $input['primary_color'] ?? '' ) ?: '#2783DE';
        $out['position'] = in_array( $input['position'] ?? 'right', array( 'left','right' ), true ) ? $input['position'] : 'right';
        ...
        $out['retention_days'] = max( 1, min( 3650, absint( $input['retention_days'] ?? 30 ) ) );
        return $out;
    }
    public static function api_key( array $settings ): string {
        $constant = defined( 'ORION_AI_OPENROUTER_KEY' ) ? trim( (string) ORION_AI_OPENROUTER_KEY ) : '';
        return '' !== $constant ? $constant : trim( (string) ( $settings['api_key'] ?? '' ) );
    }
}
