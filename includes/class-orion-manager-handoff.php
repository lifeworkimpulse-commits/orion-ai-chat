...
final class Orion_Manager_Handoff{
 ...
 public function all(int $limit=100):array{global$wpdb;$limit=max(1,min(500,$limit));return$wpdb->get_results($wpdb->prepare("SELECT * FROM {$this->table} ORDER BY FIELD(status,'new','resolved'),id DESC LIMIT %d",$limit),ARRAY_A)?:array();}
 public function resolve(int $id):bool{global$wpdb;return false!==$wpdb->update($this->table,array('status'=>'resolved','updated_at'=>current_time('mysql',true)),array('id'=>$id));}
}
