...
final class Orion_AI_Assistant {
    ...
    private function __construct() {
        ...
        $knowledge = new Orion_Knowledge_Base();
        $products = new Orion_Product_Search();
        $calculator = new Orion_Ceiling_Calculator();
        $conversations = new Orion_Conversation_Service();
        $rate_limiter = new Orion_Rate_Limiter();
    ...
    public static function activate(): void {
        self::install_schema();
        if (get_option(Orion_AI_Settings::OPTION, null) === null) {
            add_option(Orion_AI_Settings::OPTION, Orion_AI_Settings::defaults());
        }
    }

    public static function maybe_upgrade(): void {
        if (get_option('orion_ai_schema_version') !== ORION_AI_SCHEMA_VERSION) self::install_schema();
    }
    ...
    private static function install_schema(): void {
        ...
        dbDelta("CREATE TABLE {$wpdb->prefix}orion_ai_documents (
        ...
        dbDelta("CREATE TABLE {$wpdb->prefix}orion_ai_conversations (
            ...
            created_at DATETIME NOT NULL,
            PRIMARY KEY (id),
            KEY client_day (client_hash, day_key),
            UNIQUE KEY session_key (session_key)
        ) $charset;");
        dbDelta("CREATE TABLE {$wpdb->prefix}orion_ai_messages (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
        ...
        dbDelta("CREATE TABLE {$wpdb->prefix}orion_ai_events (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    public function enqueue_widget(): void {
        ...
        wp_localize_script('orion-ai-widget', 'OrionAI', array(
            'rest' => esc_url_raw(rest_url('orion-ai/v1/')),
            ...
            'questionsPerSession' => (int) $settings['questions_per_session'],
            'quickPrompts' => array('Help me choose products', 'I need materials for a ceiling', 'Delivery and returns'),
        ));
    }
}
